...
then
  ...
  # ---------------------------------------------------------------------------
  # Same as before, as test build before adding new L4+ NOR FLASH.
...
then
  ...
  # ---------------------------------------------------------------------------
  # Same as before, as test build for adding STM32G0 series NOR FLASH.

  OPENOCD_VERSION="0.10.0-12"

  OPENOCD_GIT_BRANCH=${OPENOCD_GIT_BRANCH:-"gnu-mcu-eclipse-dev"}
  OPENOCD_GIT_COMMIT=${OPENOCD_GIT_COMMIT:-"20b0eca0490fbc4f38f69eed8542cb082b354b03"}
